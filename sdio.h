...
/* /1* CEATAEND: CE-ATA command completion signal received for CMD61 *1/ */
/* #define SDIO_STA_CEATAEND		(1 << 23) */

/* /1* SDIOIT: SDIO interrupt received *1/ */
/* #define SDIO_STA_SDIOIT			(1 << 22) */

/* /1* RXDAVL: Data available in receive FIFO *1/ */
/* #define SDIO_STA_RXDAVL			(1 << 21) */

/* /1* TXDAVL: Data available in transmit FIFO *1/ */
/* #define SDIO_STA_TXDAVL			(1 << 20) */

/* /1* RXFIFOE: Receive FIFO empty *1/ */
/* #define SDIO_STA_RXFIFOE		(1 << 19) */

/* /1* TXFIFOE: Transmit FIFO empty *1/ */
/* /1* HW Flow Control enabled -> TXFIFOE signals becomes activated when the FIFO */
/*  * contains 2 words. */
/*  *1/ */
/* #define SDIO_STA_TXFIFOE		(1 << 18) */

/* /1* RXFIFOF: Receive FIFO full *1/ */
/* /1* HW Flow Control  enabled => RXFIFOF signals becomes activated 2 words before */
/*  * the FIFO is full. */
/*  *1/ */
/* #define SDIO_STA_RXFIFOF		(1 << 17) */

/* /1* TXFIFOF: Transmit FIFO full *1/ */
/* #define SDIO_STA_TXFIFOF		(1 << 16) */

/* /1* RXFIFOHF: Receive FIFO half full: there are at least 8 words in the FIFO *1/ */
/* #define SDIO_STA_RXFIFOHF		(1 << 15) */

/* /1* TXFIFOHE: Transmit FIFO half empty: at least 8 words can be written into */
/*  * the FIFO */
/*  *1/ */
/* #define SDIO_STA_TXFIFOHE		(1 << 14) */

/* /1* RXACT: Data receive in progress *1/ */
/* #define SDIO_STA_RXACT			(1 << 13) */

/* /1* TXACT: Data transmit in progress *1/ */
/* #define SDIO_STA_TXACT			(1 << 12) */

/* /1* CMDACT: Command transfer in progress *1/ */
/* #define SDIO_STA_CMDACT			(1 << 11) */

/* /1* DBCKEND: Data block sent/received (CRC check passed) *1/ */
/* #define SDIO_STA_DBCKEND		(1 << 10) */

/* /1* STBITERR: Start bit not detected on all data signals in wide bus mode *1/ */
/* #define SDIO_STA_STBITERR		(1 << 9) */

/* /1* DATAEND: Data end (data counter, SDIDCOUNT, is zero) *1/ */
/* #define SDIO_STA_DATAEND		(1 << 8) */

/* /1* CMDSENT: Command sent (no response required) *1/ */
/* #define SDIO_STA_CMDSENT		(1 << 7) */

/* /1* CMDREND: Command response received (CRC check passed) *1/ */
/* #define SDIO_STA_CMDREND		(1 << 6) */

/* /1* RXOVERR: Received FIFO overrun error *1/ */
/* #define SDIO_STA_RXOVERR		(1 << 5) */

/* /1* TXUNDERR: Transmit FIFO underrun error *1/ */
/* #define SDIO_STA_TXUNDERR		(1 << 4) */

/* /1* DTIMEOUT: Data timeout *1/ */
/* #define SDIO_STA_DTIMEOUT		(1 << 3) */

/* /1* CTIMEOUT: Command response timeout *1/ */
/* #define SDIO_STA_CTIMEOUT		(1 << 2) */

/* /1* DCRCFAIL: Data block sent/received (CRC check failed) *1/ */
/* #define SDIO_STA_DCRCFAIL		(1 << 1) */

/* /1* CCRCFAIL: Command response received (CRC check failed) *1/ */
